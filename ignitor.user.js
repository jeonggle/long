...
!(async function () {
  ...
  class CrackMessageSender extends PlatformMessageSender {
    ...
    async send(message) {
      ...
      await CrackUtil.chatRoom().changeChatModel(this.chatRoomId, normalChatModel.id);
      const result = await CrackUtil.chatRoom().send(this.chatRoomId, "OOC: '1' 하나만 응답할 것.", {
        onMessageSent: async (user, bot) => {
          await CrackUtil.chatRoom().editMessage(this.chatRoomId, user.id, message);
          replyEditor = async (msg) => {
            await CrackUtil.chatRoom().editMessage(this.chatRoomId, bot.id, msg);
          };
        },
      });
      await CrackUtil.chatRoom().changeChatModel(this.chatRoomId, originChatId.modelId);
